...
eco_ecoregion <- rasterize(ecoregion, fl_raster, field="ECO_NAME", touches=TRUE)
...
ecoregion_num <- as.data.frame(eco_ecoregion, cells=TRUE, na.rm=TRUE)
ecoregion_num.clean <- ecoregion_num[ecoregion_num$cell %in% cell.id,]

library(dplyr)
df.ecozone <- realm_num.clean %>%
  left_join(nnh_num.clean, by = "cell") %>%
  left_join(biome_num.clean, by = "cell") %>%
  left_join(ecoregion_num.clean, by="cell")
